...
def _read_cache() -> dict:
    try:
        ...
        if "entries" not in cache:
            cache = {"entries": cache, "metadata": {"created": time.time(), "size": len(content)}}
        return cache
    except Exception:
        return {"entries": {}, "metadata": {"created": time.time(), "size": 0}}
...
def _clean_cache(cache: dict) -> dict:
    ...
    if cache["metadata"]["size"] > CACHE_SIZE_LIMIT:
        ...
        sorted_entries = sorted(cleaned_entries.items(), key=lambda x: x[1].get("ts", 0))
...
def update_profile_from_trip(profile: TravelerProfile, trip_data: dict[str, Any]) -> None:
        ...
        for attraction_id, rating in ratings.items():
            ...
            if attraction_type == "museum":
                profile.likes_museums = max(
                    0, min(1, profile.likes_museums + normalized_rating * learning_factor)
                )
            elif attraction_type == "nature":
                profile.likes_nature = max(
                    0, min(1, profile.likes_nature + normalized_rating * learning_factor)
                )
            elif attraction_type == "nightlife":
                profile.likes_nightlife = max(
                    0, min(1, profile.likes_nightlife + normalized_rating * learning_factor)
                )
                ...
                profile.likes_restaurants = max(
                    0, min(1, profile.likes_restaurants + normalized_rating * learning_factor)
                )
...
def _categorize_attractions(attractions: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for attraction in attractions:
        ...
        if any(t in ["museum", "art_gallery", "library"] for t in types):
            ...
            category = "food"
        elif any(t in ["church", "mosque", "synagogue", "temple", "historical"] for t in types):
            category = "historical"
...
def get_contextual_recommendations(
    ...
    for attraction in attractions:
        ...
        types = attraction.get("types", [])
        is_indoor = any(t in ["museum", "shopping_mall", "restaurant", "theater"] for t in types)
        is_outdoor = any(t in ["park", "beach", "hiking_area", "viewpoint"] for t in types)
        ...
        if time_of_day == "morning" and attraction_type == "museum":
            score += 0.2
        elif time_of_day == "evening" and attraction_type in ["restaurant", "nightlife"]:
            score += 0.2

        # Crowded places tolerance
        is_popular = attraction.get("rating", 0) > 4.5 and attraction.get("reviews", 0) > 1000
        if is_popular:
...
def generate_personalized_narrative(
    profile: TravelerProfile, city: str, attractions: list[dict[str, Any]], style: str = "friendly"
) -> str:
    ...
    style_transitions = {
        ...
        "professional": ["Kolejny punkt", "Następnie", "Zgodnie z planem", "Optymalna kolejność"],
    }
...
def _generate_attraction_narrative(
    ...
    attraction_type = _categorize_attractions([attraction]).get(attraction.get("id", ""), "unknown")
    personal_note = ""
...
def html_export(title: str, body_md: str) -> str:
    def md2html(md: str) -> str:
        ...
        t = re.sub(
            r"\n\|([^\n]+)\|\n\|([\-:\| ]+)\|\n((?:\|.*\|\n?)+)",
            (r"\n<table><thead><tr><th>\1</th></tr></thead>" r"<tbody>\3</tbody></table>"),
            t,
        )
        return t.replace("\n\n", "<br/><br/>")
...
def greedy_cluster(
    ...
    work = items[:]
    if center:
        work.sort(key=lambda it: haversine(center, (it.get("lat") or 0.0, it.get("lon") or 0.0)))
...
class TravelerProfile:
    ...
    learning_scores: dict[str, float] | None = None  # np. {"museums": 0.8, "nature": 0.3}
...
def adapt_otm_place(item: dict[str, Any]) -> UnifiedPlace:
    ...
    return UnifiedPlace(
        ...
        lat=item.get("lat"),
        lon=item.get("lon"),
        url=item.get("url", "") or f"https://www.opentripmap.com/en/card/{item.get('xid', '')}",
...
def overpass_poi(
    ...
    return [x for x in out if x.get("lat") and x.get("lon") and (x.get("name") or "").strip()]
            ...
            [
                {
                    ...
                    "map_url": (
                        "https://www.google.com/maps/place/?q=place_id:" f"{it.get('place_id','')}"
                        if it.get("place_id")
                        else ""
                    ),
                }
...
def g_details(place_id: str, lang: str = "pl") -> dict[str, Any]:
    ...
    r = _get(
        ...
        params={
            ...
            "key": GOOGLE,
            "language": lang,
            "fields": "formatted_phone_number,opening_hours,website," "price_level,photos",
        },
        source="google",
    )
    if r.status_code >= 400:
...
def ta_map(lat: float, lon: float, category: str, limit: int = 40) -> list[dict[str, Any]]:
    if not TA_KEY:
        return []
...
def merge_place_sources(*source_lists: tuple[list[UnifiedPlace], str]) -> list[UnifiedPlace]:
    """
...
def restaurants(
    ...
    # Google Places
    google_raw = restaurants_google(place, q, max_results, open_now, lang) if GOOGLE else []
    google_places = unify_places(google_raw, "google")

    # TripAdvisor
    ta_raw = ta_restaurants(geo["lat"], geo["lon"], max_results) if geo and TA_KEY else []
    ta_places = unify_places(ta_raw, "tripadvisor")

    # Merge premium sources
    if google_places or ta_places:
        merged = merge_place_sources((google_places, "google"), (ta_places, "tripadvisor"))[
            :max_results
        ]
...
def attractions(place: str, max_results: int = 80, lang: str = "pl") -> list[dict[str, Any]]:
    geo = geocode(place)
    if not geo:
        return []
        ...
        if place.name:
            ...
            if wiki:
                ...
                enriched_place = UnifiedPlace(
                    ...
                    photos=place.photos,
                    description=wiki.get("extract", place.description) or place.description,
                    source=place.source,
                    types=place.types,
                )
...
def tl_stops_near(lat: float, lon: float, radius_m: int = 900) -> list[dict[str, Any]]:
    ...
    r = _get("https://transit.land/api/v2/rest/stops", params=params, source="transitland")
    if r.status_code >= 400:
        return []
    return [
        {
...
def estimate_cost(
    ...
    if any(region in place_lower for region in ["norway", "switzerland", "denmark", "sweden"]):
        base_low, base_high = int(base_low * 1.8), int(base_high * 2.2)  # Nordic premium
    elif any(region in place_lower for region in ["poland", "czech", "hungary", "slovakia"]):
        base_low, base_high = int(base_low * 0.6), int(base_high * 0.8)  # Eastern Europe
    elif any(region in place_lower for region in ["spain", "portugal", "greece"]):
        base_low, base_high = int(base_low * 0.8), int(base_high * 1.1)  # Southern Europe
    elif any(region in place_lower for region in ["germany", "france", "netherlands", "belgium"]):
        base_low, base_high = int(base_low * 1.2), int(base_high * 1.5)  # Western Europe
...
def get_schedule_template(mood: str = "spokój", trip_type: str = "standard") -> dict[str, Any]:
...
def _item_md(it: dict[str, Any], day_idx: int) -> str:
    ...
    url = (
        it.get("url")
        or ((it.get("wiki") or {}).get("url") if isinstance(it.get("wiki"), dict) else "")
        or it.get("map_url", "")
        or it.get("website", "")
    )
    desc = it.get("desc") or (
        (it.get("wiki") or {}).get("extract") if isinstance(it.get("wiki"), dict) else ""
    )
...
def plan_trip(
    ...
    if poi and profile:
        ...
        time_of_day = (
            "morning" if current_hour < 12 else ("afternoon" if current_hour < 18 else "evening")
        )
    ...
    hotels_l = hotels(spec.place, "", max_results=24, lang=spec.lang) if spec.hotels else []
    ...
    if wx:
        ...
        for d in wx[: spec.days]:
            if "narrative" in d:
                ...
                md.append(
                    f"- {d['day']}: {d['tmin']}-{d['tmax']}°C • opady " f"{d.get('precip',0)} mm"
                )
        md.append("")
    ...
    def _fmt_item(x: dict[str, Any]) -> str:
        price = ("$" * int(x["price_level"])) if (x.get("price_level") is not None) else ""
        line = (
    ...
    if stops:
        ...
        for s in stops[:12]:
            md.append(f"- {s['name']} — {s['onestop_id']} " f"({s['lat']:.4f},{s['lon']:.4f})")
        md.append("")
...
def _main(argv: list[str]) -> int:
    ...
    ap = argparse.ArgumentParser(description="TravelGuide ULTRA — Google+TripAdvisor+OTM/OSM")
    ...
    p.add_argument(
        "--style",
        default="auto",
        choices=["auto", "friendly", "elegant", "energetic", "scholarly", "casual", "professional"],
        help="Styl narracji",
    )
    ...
    if args.cmd == "food":
        _print(
            {
                "place": args.place,
                "items": restaurants(args.place, args.q, args.max, args.open_now, args.lang),
            }
        )
        return 0
    if args.cmd == "hotels":
    ...
    if args.cmd == "attractions":
        ...
        return 0
    if args.cmd == "flights":
        o = args.frm.upper() if len(args.frm) == 3 else (tp_iata(args.frm) or args.frm.upper())
        d = args.to.upper() if len(args.to) == 3 else (tp_iata(args.to) or args.to.upper())
        _print(
            {
                "origin": o,
                "dest": d,
    ...
    if args.cmd == "map":
        ...
        for h in hotels_l[:3]:
            if h.get("lat"):
                pins.append((h["lat"], h["lon"], "H"))
        url = static_map_url((geo["lat"], geo["lon"]), pins) or "(brak mapy — brak GOOGLE_MAPS_KEY)"
