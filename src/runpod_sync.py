...
class RunPodSync:
    ...
    def __init__(self):
        ...
        self.local_db_path = Path(
            os.path.join(Path(__file__).parent, "data", "memory.db")
        )
    ...
    def _create_backup(self) -> bool:
        ...
        try:
            ...
            for suffix in ["-wal", "-shm"]:
                local_side_file = self.local_db_path.with_name(
                    self.local_db_path.name + suffix
                )
                remote_side_file = self.remote_db_path.with_name(
                    self.remote_db_path.name + suffix
                )
            ...
            logger.info(
                f"Kopia zapasowa bazy danych utworzona w RunPod: {self.remote_db_path}"
            )
            return True
    ...
    def _restore_from_backup(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Baza danych odtworzona z kopii zapasowej RunPod: {self.remote_db_path}"
            )
            return True
    ...
    def check_pod_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Błąd API RunPod: {response.status_code} - {response.text}"
                )
                return {
                    "status": "error",
    ...
    def start_sync(self):
        ...
        if not self.use_runpod:
            logger.info(
                "RunPod nie jest włączony, synchronizacja nie została uruchomiona"
            )
            return
...
if __name__ == "__main__":
    ...
    if args.command == "start":
        ...
        print(
            f"Synchronizacja {'zakończona pomyślnie' if success else 'nie powiodła się'}"
        )
        ...
        print(
            f"Odtworzenie {'zakończone pomyślnie' if success else 'nie powiodło się'}"
        )
