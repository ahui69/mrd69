...
class EventBus:
    ...
    def _rotate_if_needed(self):
        try:
            if self.log_path.exists() and self.log_path.stat().st_size > ROTATE_MB * 1024 * 1024:
                ts = int(time.time())
                dst = self.log_path.with_name(f"events_{ts}.jsonl.gz")
                with open(self.log_path, "rb") as src, _gzip.open(dst, "wb", compresslevel=5) as gz:
                ...
                if autopilot is not None and callable(autopilot):
                    ...
                    if res:
                        BUS.publish(
                            "memo",
                            "note",
                            {"title": "[PSY]autopilot", "sources": [{"text": str(res)}]},
                            ev.user,
                        )
...
# === HMAC (API) ===
def _sig_base(ts_ms: int, channel: str, type_: str, user: str, data: dict[str, Any]) -> bytes:
    payload = json.dumps(data or {}, ensure_ascii=False, separators=(",", ":"))
    s = f"{int(ts_ms)}.{channel}.{type_}.{user}.{payload}"
    return s.encode("utf-8")


def make_hmac(ts_ms: int, channel: str, type_: str, user: str, data: dict[str, Any]) -> str:
    if not AUTH_TOKEN:
        return ""
    return hmac.new(
...
def _main(argv: list[str]) -> int:
    ...
    if args.cmd == "export":
        ...
        return 0
    if args.cmd == "import":
        print(json.dumps({"imported": import_jsonl(args.path)}, ensure_ascii=False, indent=2))
        return 0
    return 0
