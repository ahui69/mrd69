...
def _cache_set(url: str, params: dict[str, Any] | None, data: Any, ttl: int | None = None) -> None:
    if _CACHE_TTL <= 0:
        return
    k = _ck(url, params)
...
def cg_chart(id_: str, days: int = 1, vs: str = "usd", interval: str = "minute") -> dict:
    return _get_json(
...
def binance_klines(symbol: str, interval: str = "1m", limit: int = 120) -> list[list[Any]]:
    params = {
        "symbol": symbol.upper(),
        "interval": interval,
...
def _es_call(params: dict[str, Any]) -> dict[str, Any]:
    ...
    r = HTTP.get(ES_BASE, params=params, timeout=int(os.getenv("ETHERSCAN_TIMEOUT_S", "20")))
    r.raise_for_status()
    return r.json()
...
def es_native_balance(address: str) -> float | None:
    if not address:
        return None
    j = _es_call({"module": "account", "action": "balance", "address": address, "tag": "latest"})
    try:
...
def support_resistance(
    ...
    for v in seg:
        ...
        if abs(v - M) / max(1e-9, M) < eps:
            r.append(v)
    return (
        sorted(set(round(x, 8) for x in s))[:5],
        sorted(set(round(x, 8) for x in r))[:5],
    )
...
def screener_top(n: int = 30, vs: str = "usd") -> list[dict[str, Any]]:
    ...
    for co in mk:
        try:
            ...
            risk = min(10, max(1, 4 + (1 if rsi14 > 70 else 0) + (1 if vol48 > 0.08 else 0)))
            last24 = prices[-24:]
...
def portfolio_plan(meta: dict, horizon: str, risk_profile: str) -> dict:
    ...
    raw *= 1.0 + (0.12 if (pump >= 60 and pump < 85) else (-0.12 if pump >= 85 else 0.0))
...
def advice_text(meta: dict, horizon: str, risk_profile: str, up_only_bias: bool = False) -> str:
...
def _persist_fact(text: str, tags: list[str] | None = None, conf: float = 0.6) -> str | None:
    m = _mem()
    if not m:
        return None
    try:
        return m.add_fact(text.strip(), tags=sorted(set((tags or []) + ["crypto"])), conf=conf)
    except Exception:
        return None
                ...
                ev.append(
                    {
                        ...
                        "value_eth": val_eth,
                        "direction": (
                            "in" if tx.get("to", "").lower() == address.lower() else "out"
                        ),
                    }
                )
        except Exception:
...
def portfolio_watch_loop(user_id: str, interval_sec: int = 300, threshold_pct: float = 5.0):
    ...
    while True:
        try:
            ...
            for pos in updated_portfolio.positions:
                old_pos = next((p for p in portfolio.positions if p.symbol == pos.symbol), None)
            ...
            if abs(updated_portfolio.performance_24h or 0) >= threshold_pct:
                direction = "🚀" if (updated_portfolio.performance_24h or 0) > 0 else "💥"
                _alert(
...
def load_portfolio(user_id: str) -> Portfolio:
    ...
    return Portfolio(user_id=user_id, name=f"Portfolio {user_id}", addresses=[], positions=[])
        ...
        try:
            ...
            if eth_balance and eth_balance > 0:
                ...
                position = PortfolioPosition(
                    ...
                    price_usd=eth_price,
                    price_change_24h=eth_data.get("ethereum", {}).get("usd_24h_change", 0.0),
            ...
            log_line("error", {"where": "portfolio_scan", "address": address, "err": str(e)})
...
def portfolio_performance_report(portfolio: Portfolio) -> dict[str, Any]:
    ...
    best_performer = max(portfolio.positions, key=lambda p: p.price_change_24h, default=None)
    worst_performer = min(portfolio.positions, key=lambda p: p.price_change_24h, default=None)

    report = {
        "portfolio_id": portfolio.user_id,
        "total_value": {"usd": portfolio.total_value_usd, "pln": portfolio.total_value_pln},
        ...
        "performance_24h": {
            ...
            "worst": {
                ...
                "change_pct": worst_performer.price_change_24h if worst_performer else 0,
            },
        },
...
def calculate_token_score(token_id: str, vs: str = "usd") -> dict[str, Any]:
    ...
    try:
        # Basic data
        coin_data = cg_simple_price(ids=token_id, include_24hr_vol=True, include_market_cap=True)
        ...
        composite_score = liquidity_score * 0.4 + trust_score * 0.3 + community_score * 0.3
        ...
        return {
            ...
            "risk_level": (
                "low" if composite_score > 80 else ("medium" if composite_score > 50 else "high")
            ),
            "last_updated": _now_iso(),
        }
...
def simple_sma_strategy_backtest(
    ...
    try:
        ...
        for i in range(min(len(sma_short), len(sma_long))):
            ...
            current_value = temp_capital + (temp_position * price if temp_in_position else 0)
...
def _cli():
    ...
    s6.add_argument("--interval", type=int, default=300, help="Watch interval in seconds")
    ...
    if args.cmd == "screener":
        ...
        paths = make_report(args.id_or_sym, horizon=args.horizon, risk_profile=args.risk)
