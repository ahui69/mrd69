...
def db_upsert_thread(chat_id: str, title: Optional[str]) -> None:
    ...
    with _conn() as c:
        ...
        if row is None:
            c.execute(
                "INSERT INTO chat_threads(id, title, created_ts, updated_ts) VALUES(?,?,?,?)",
...
def db_get_chat(chat_id: str) -> Dict[str, Any]:
    with _conn() as c:
        msgs = c.execute(
            "SELECT role, content, ts FROM chat_messages WHERE chat_id=? ORDER BY ts ASC", (chat_id,)
        ).fetchall()
...
def db_list_chats(limit: int = 200) -> List[Dict[str, Any]]:
    with _conn() as c:
        rows = c.execute(
            "SELECT id, title, updated_ts FROM chat_threads ORDER BY updated_ts DESC LIMIT ?", (limit,)
        ).fetchall()
...
# Frontend – jeśli istnieje ./frontend, montujemy pod /app (żeby /api/* było wolne)
...
def api_health():
    ...
    mem_ok = False
    try:
        # jeżeli router pamięci zamontowany, powinien odpowiadać na /api/memory/health
        mem_ok = True
    except Exception:
        mem_ok = False
    return {"ok": True, "mode": "echo" if llm is None else "llm", "memory_router": mem_ok}
...
async def api_upload(file: UploadFile = File(...)):
    out = os.path.join("data", "uploads", f"{int(time.time()*1000)}_{file.filename}")
...
def api_chat(req: ChatRequest):
    try:
        ...
        if req.newMessage:
            ...
            db_add_message(chat_id, req.newMessage.get("role", "user"), req.newMessage.get("content", ""))
