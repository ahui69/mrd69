    ...
    def get_status(self) -> dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "rejection_rate": sum(self.rejection_stats.values()) / max(1, self.total_requests),
                "kind_cooldowns": {
...
class ActionIdempotency:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            duplicate_rate = sum(max(0, data["count"] - 1) for data in self.actions.values()) / max(
                1, sum(data["count"] for data in self.actions.values())
            )
            ...
            return {
                ...
                "avg_confidence": sum(data["confidence"] for data in self.actions.values())
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        ...
                        error_type = (
                            "LLM" if "llm" in str(e).lower() or "openai" in str(e).lower() else "IO"
                        )
                        print(f"🔄 Retry {attempt + 1}/{max_retries} dla {func.__name__}: {e}")
                        print(f"⏳ Czekam {delay:.1f}s przed kolejną próbą...")
                        time.sleep(delay)
                    else:
                        print(f"❌ {func.__name__} nie powiódł się po {max_retries} próbach: {e}")
                        raise e
...
class TelemetryCollector:
    ...
    def record_proposals(self, telemetry: TickTelemetry, proposals: list[dict]):
        ...
        if proposals:
            telemetry.avg_proposal_score = sum(p.get("score", 0.5) for p in proposals) / len(
                proposals
            )
    ...
    def record_error(self, telemetry: TickTelemetry, error: Exception, context: str):
        """Zapisuje błąd."""
        telemetry.errors.append({"error": str(error), "context": context, "timestamp": time.time()})
    ...
    def _calculate_batch_metrics(self, batch: list[dict]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_proposals": sum(t.get("proposals_count", 0) for t in batch) / len(batch),
            "avg_acceptance_rate": sum(t.get("acceptance_rate", 0) for t in batch) / len(batch),
    ...
    def get_rolling_metrics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_proposals_per_tick": sum(m.get("avg_proposals", 0) for m in recent) / len(recent),
            ...
            "avg_tick_duration": sum(m.get("avg_duration", 0) for m in recent) / len(recent),
...
class ReliableAISystem:
    ...
    def process_critical_operation(
        ...
        try:
            ...
            result = self._execute_monitored_operation(operation_type, telemetry, **kwargs)
    ...
    def _monitored_chat_completion(self, telemetry: TickTelemetry, **kwargs) -> dict[str, Any]:
    ...
    def _monitored_memory_operation(self, telemetry: TickTelemetry, **kwargs) -> dict[str, Any]:
    ...
    def _monitored_decision_making(self, telemetry: TickTelemetry, **kwargs) -> dict[str, Any]:
...
def reliable_operation(operation_type: str = "general"):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            operation_kwargs = {"original_function": func.__name__, "args": args, "kwargs": kwargs}
