...
AUTO_TAGS = [
    t.strip() for t in os.getenv("AUTO_TAGS", "autonauka,web,evidence").split(",") if t.strip()
]
...
TAG_SCRUB = re.compile(r"<script[^>]*>.*?</script>|<style[^>]*>.*?</style>", re.I | re.S)
...
# ====== LLM (jedno) ======
def _llm_chat(system: str, user: str, maxtok: int = 512, temp: float = 0.1) -> str | None:
    base = LLM_BASE
    key = LLM_KEY
    model = LLM_MODEL
...
def _s2_search(query: str, k: int = 5) -> list[dict]:
    try:
        r = HTTP.get(
            ...
            params=_params({"query": query, "limit": k, "fields": "title,url,abstract"}),
            timeout=WEB_HTTP_TIMEOUT,
        )
        j = r.json()
        return [
...
def _save_profile(hashes: list[str], domain_weights: dict[str, float]) -> None:
    try:
        ...
        prof["web_domain_weights"] = {k: round(float(v), 3) for k, v in domain_weights.items()}
...
def _edit_facts(facts: list[str]) -> list[str]:
    if not facts:
        return facts
    sys = "Przepisz fakty w krótkie, jasne zdania, usuń dublowanie. Zwróć listę punktów."
    usr = "\n".join(f"- {f}" for f in facts)
    out = _llm_chat(sys, usr, maxtok=256, temp=0.0) or ""
    if not out:
        return facts
    lines = [re.sub(r"^[\\-\\*\\d\\.\\)\\s]+", "", line).strip() for line in out.splitlines()]
...
def _pipeline_serp_like(
    ...
    if len(mats) < max_fetch:
        ...
        for h in hits:
            ...
            consider({"link": u, "title": s.get("title", ""), "snippet": s.get("extract", "")})
            if len(mats) >= max_fetch:
                break
...
def _pipeline_free(
    ...
    if len(mats) < max_fetch:
        ...
        for h in hits:
            ...
            seen.add(k)
            txt = s["extract"]
            chs = [c for c, _ in _rank_jaccard(query, _chunks(txt, target=700, overlap=100))]
    ...
    mats.sort(
        key=lambda m: 0.75 * float(m.get("trust", 0)) + 0.25 * float(m.get("recency", 0)),
        reverse=True,
    )
    return mats[:max_fetch]
...
"""
    ...
    try:
        ...
        if r.status_code < 400:
            j = r.json()
            return j.get("choices", [{}])[0].get("message", {}).get("content", "").strip() or ""
    except Exception:
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="autonauka — monolit (jedno LLM 4B, bez Google CSE)")
    p.add_argument("query")
    p.add_argument("--mode", choices=["full", "free", "fast", "grounded"], default="grounded")
