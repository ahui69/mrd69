...
structlog.configure(
    ...
    wrapper_class=structlog.make_filtering_bound_logger(getattr(structlog.stdlib.logging, config.LOG_LEVEL, 20)),
...
import autonauka, crypto_advisor_full, file_client, kimi_client, memory, programista, psychika, runpod_sync, search_client, travelguide, writing_all_pro
...
async def lifespan(app: FastAPI):
    ...
    # Ładowanie wiedzy z plików
    load_knowledge_from_jsonl("data/memory.jsonl")
    load_knowledge_from_jsonl("data/start.seed.jsonl") # Ładowanie Twojej wiedzy
            ...
            for line in f:
                try:
                    ...
                    if text := data.get("text"):
                        mem.add_fact(text, conf=0.95, tags=["wiedza_jsonl"] + data.get("tags", []))
                except json.JSONDecodeError:
                    log.warning("Pominięto nieprawidłową linię JSON", file_path=file_path)
...
# === Monitoring i Zdrowie (Punkt 6) ===
@app.get("/health")
async def health():
    # Proste sprawdzenie, czy kluczowe systemy są zainicjalizowane
    checks = {
...
async def version():
    return {
        "version": "3.0.0", # Podniesiona wersja
        "git_sha": get_git_commit_sha(),
        "build_date": os.getenv("BUILD_DATE", "unknown"),
    }

# === API DLA ZAAWANSOWANEJ PAMIĘCI (Punkt 8) ===
...
# === WebSocket (przeniesiony i oczyszczony) ===
@app.websocket("/ws")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # Użycie _chat_llm do przetworzenia wiadomości
            response_text = _chat_llm(msg, "") # Kontekst można dodać w przyszłości
...
# === Logika LLM (przeniesiona i oczyszczona) ===
...
def _chat_llm(user_text: str, ctx_text: str) -> str:
    ...
    try:
        return kimi_client.kimi_chat(msgs, max_tokens=config.CHAT_MAX_TOKENS, temperature=config.CHAT_TEMPERATURE)
    except Exception as e:
        log.error("Kimi API error, fallback to Qwen", error=str(e))
        return psychika._llm_chat(msgs, maxtok=config.CHAT_MAX_TOKENS, temp=config.CHAT_TEMPERATURE) or "Nie mam pewnej odpowiedzi."
...
def _handle_knowledge_loading_command(user_text: str) -> str:
    ...
    return f"✅ Załadowano: {result['added']} | Połączono: {result['merged']}" if result["ok"] else f"❌ Błąd: {result['reason']}"

# === Pozostałe Endpoints (przeniesione i oczyszczone) ===
...
async def create_upload_file(files: list[UploadFile]):
    ...
    for f in files:
        try:
            ...
            if not result.ok: raise Exception(result.err)
            ...
            if text_result.ok and text_result.data.get("text"):
                mem.add_entry(f"Zawartość pliku {f.filename}:\n{text_result.data['text']}", metadata={"source": "file_upload", "filename": f.filename})
...
async def modules():
    try:
        ...
        has_images = False
    return {
        "autonauka": hasattr(autonauka, "web_learn"), "psychika": True, "memory": True,
        "writer": hasattr(writing_all_pro, "assist"), "travelguide": hasattr(travelguide, "plan"),
        "images": has_images, "files": hasattr(file_client, "process_file"),
        "crypto": hasattr(crypto_advisor_full, "analyze"), "programista": hasattr(programista, "write_code"),
        "runpod": config.USE_RUNPOD, "knowledge_loader": hasattr(mem, "load_knowledge"),
    }

# --- RunPod Integration ---
@app.get("/runpod/status")
async def runpod_status():
    if not config.USE_RUNPOD: return {"status": "disabled"}
...
async def force_runpod_backup():
    if not config.USE_RUNPOD: return JSONResponse(status_code=400, content={"error": "disabled"})
...
async def restore_from_runpod():
    if not config.USE_RUNPOD: return JSONResponse(status_code=400, content={"error": "disabled"})
    return {"status": "success"} if runpod_sync.get_runpod_sync()._restore_from_backup() else JSONResponse(status_code=500, content={"status": "error"})

# --- Knowledge Loader API ---
class KnowledgeTextData(BaseModel): content: str
class KnowledgeURLData(BaseModel): url: str
class KnowledgeFileData(BaseModel): file_path: str
...
# --- MORDZIX API ---
class MordzixChatRequest(BaseModel): thread_id: str | None = None; user_id: str; content: str
